...
def SaveEvalData(model, env, fileName, steps, oginstrcount, benchmark, obsspace):
    ...
    for i in range (1, episodes + 1 ):
        ...
        if (env.observation["Runtime"] == None):
                runtime = None
        elif (len(env.observation["Runtime"] > 0)):
            runtime = env.observation["Runtime"][0]
        else:
            runtime = None
        ...
        f1data.append(fileName + ','+ str(steps) + ','+ str(env.observation["IrInstructionCount"]) + ','+ str(truncate(score,4)) + ','+ str(truncate(int(oginstrcount) / instCnt,2))+','+str(runtime)+'\n')
   

    #inst data sets
    #algo, step, best_instcount, worst_count, avg_count
    f2data.append(fileName + ','+ str(steps) + ','+ str(best_inst_cnt) + ','+ str(worst_inst_cnt) + ','+str(truncate(running_inst_cnt / episodes,2))+','+str(runtime)+'\n')

    #inst data sets
    #algo, step, best_instcount, worst_count, avg_count
    f3data.append(fileName + ','+ str(steps) +','+ str(truncate(score,4)) + ','+ str(truncate(score,4)) + ','+ str(truncate(running_rewards / episodes,2))+','+str(runtime)+'\n')
...
def CreateModelAndLearn(net_arch_enabled, net_arch, algorithm, maxsteps, pbenchmark, observataionSpace, stepvalue):
    ...
    #define a model
    if (algorithm == 'PPO'):
        ##{'learning_rate': 0.00021587401172277018, 'n_steps': 768, 'gamma': 0.904995863424501, 'gae_lambda': 0.886880966782373}
    ...
    if (algorithm == 'A2C'):
        model = A2C('MlpPolicy', wrapped_env_normal, verbose=0)
    ...
    while timestep_count <= maxsteps:
        ...
        if (timestep_count == maxsteps):
            Model_Path = os.path.join('Training','Saved_Models_GenerateBaseline',algorithm+'_'+pbenchmark+'_'+observataionSpace+ 'Model_algoComp_steps_'+str(timestep_count))
...
FinalFileWriteWithCreateMpdelAndLearn(False, 0, 'PPO', 15000,"cbench-v1/qsort","InstCount",1000)
FinalFileWriteWithCreateMpdelAndLearn(False, 0, 'PPO', 15000,"cbench-v1/sha","InstCount",1000)
##CreateModelAndLearn(False, 0, 'DQN', 60000,"cbench-v1/bzip2","Autophase",1000)

##try tomorrow
##same subset models with A2C to get a different dimension of actions!
...
def SaveEvalData(model, env, fileName, steps, oginstrcount, benchmark, obsspace, originalruntime):
    ...
    for i in range (1, episodes + 1 ):
        ...
        ##print(actions)
        try:
        # Try to access an observation value
            ...
            #full data sets
            #algo, step, instcount, reward, ratio, runtime, ogruntime
        ...
        if (layernodes > 0):
            if (tuned):
                model = A2C('MlpPolicy', wrapped_env_normal, verbose=1, policy_kwargs={"net_arch": net_arch},
                    learning_rate= 0.0001445, ent_coef=0.0976,gamma= 0.8408,n_steps= 20)
...
def FinalFileWriteWithCreateMpdelAndLearn(testname, algorithm, maxsteps, pbenchmark, observataionSpace, stepvalue, layernodes, tuned):
...
FinalFileWriteWithCreateMpdelAndLearn('A2C_25000_bzip2__instcount_plainmodelHyper', 'A2C', 25000,"cbench-v1/bzip2","InstCount",1000,0,True)
